<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd"
                   v-hasPermi="['promotion:coupon-template:create']">新增
        </el-button>
    ...
    <el-table v-loading="loading" :data="list">
      ...
      <el-table-column label="剩余数量" align="center" prop="totalCount"
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template v-slot="scope">
          <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)"
                     v-hasPermi="['promotion:coupon-template:update']">修改
          </el-button>
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)"
                     v-hasPermi="['promotion:coupon-template:delete']">删除
          </el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" :visible.sync="open" width="600px" v-dialogDrag append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="140px">
        ...
        <el-form-item v-if="form.discountType === PromotionDiscountTypeEnum.PRICE.type" label="优惠券面额"
                      prop="discountPrice">
          <el-input-number v-model="form.discountPrice" placeholder="请输入优惠金额，单位：元"
                           style="width: 400px" :precision="2" :min="0"/>
          元
        </el-form-item>
        <el-form-item v-if="form.discountType === PromotionDiscountTypeEnum.PERCENT.type" label="优惠券折扣"
        ...
        <el-form-item v-if="form.discountType === PromotionDiscountTypeEnum.PERCENT.type" label="最多优惠"
        ...
        <el-form-item v-if="form.validityType === CouponTemplateValidityTypeEnum.DATE.type" label="固定日期"
                      prop="validTimes">
          <el-date-picker v-model="form.validTimes" style="width: 240px" value-format="yyyy-MM-dd HH:mm:ss"
                          type="datetimerange"
                          :default-time="['00:00:00', '23:59:59']"/>
        </el-form-item>
        <el-form-item v-if="form.validityType === CouponTemplateValidityTypeEnum.TERM.type" label="领取日期"
                      prop="fixedStartTerm">
          第
        ...
        <el-form-item v-if="form.productScope === PromotionProductScopeEnum.SPU.scope" prop="productSpuIds">
          <el-select v-model="form.productSpuIds" placeholder="请选择活动商品" clearable size="small"
            ...
            <el-option v-for="item in productSpus" :key="item.id" :label="item.name" :value="item.id">
              ...
              <span
                style="float: right; color: #8492a6; font-size: 13px">￥{{ (item.minPrice / 100.0).toFixed(2) }}</span>
            </el-option>
          </el-select>
        </el-form-item>
      </el-form>
